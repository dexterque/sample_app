...
class UserTest < ActiveSupport::TestCase
  ...
  test "email validation should accept valid addresses" do
    validate_addresses = %W[user@example.com USER@foo.COM A_US-USER@foo.bar.org first.last@foo.jp alice+bob@baz.cn]
    validate_addresses.each do |validate_address|
      @user.email = validate_address
      assert @user.valid?, "#{validate_address.inspect} should be valid"
    end
  end
  ...
  test "password should be a minmum length" do
